...
def vit_patch_embeddings(
    ...
    *,
    ...
    pixel_values = torch.reshape(pixel_values, (1, img_c, img_h, patch_count, patch_size))
    pixel_values = torch.reshape(pixel_values, (1, img_c, patch_count, patch_size, patch_count, patch_size))
    pixel_values = torch.permute(pixel_values, (0, 1, 2, 4, 3, -1))
    pixel_values = torch.reshape(pixel_values, (1, img_c, patch_count_sq, patch_size_sq))
    pixel_values = torch.permute(pixel_values, (0, 2, 1, 3))
    pixel_values = torch.reshape(pixel_values, (1, patch_count_sq, patch_size_sq_trpl))
...
def vit_embeddings(
    ...
    *,
    ...
    # commented out lines are from the original model source, but are fused now inside the folding step in vit_patch_embeddings function
...
def vit(
    ...
    *,
    ...
    # Pooler
    pooler_output = output[0] @ parameters.pooler.dense.weight
