...
def run_demo_inference(device, input_path, num_prompts, num_inference_steps, image_size=(256, 256)):
    disable_persistent_kernel_cache()
...
def run_demo_inference_diffusiondb(device, image_size=(256, 256)):
    profiler.clear()
    disable_persistent_kernel_cache()
    num_prompts = 1
    ...
    for i in range(num_prompts):
        ...
        for i in range(len(time_step_list)):
            ...
            # predict the noise residual
            with torch.no_grad():
                profiler.start(f"stable_diffusion_model_run_for_inference_{i}")
                ttnn_noise_pred = model(
                    sample=ttnn_latent_model_input,
                    timestep=time_step_list[i],
                    encoder_hidden_states=ttnn_text_embeddings,
                ...
                profiler.end(f"stable_diffusion_model_run_for_inference_{i}")

            # perform guidance
            profiler.start(f"tt_guide_runtime_{i}")
            noise_pred = tt_guide(ttnn_noise_pred, guidance_scale)
            profiler.end(f"tt_guide_runtime_{i}")

            profiler.start(f"ttnn_scheduler_run_{i}")
            ttnn_latents = ttnn_scheduler.step(
                model_output=noise_pred,
                sample=ttnn_latents,
                sigma=float(ttnn_sigma[i]),
            ...
            profiler.end(f"ttnn_scheduler_run_{i}")
            save_image_and_latents(ttnn_latents, i, vae, pre_fix=f"{experiment_name}_tt", pre_fix2="")
        ...
        # get runtime profile
        for i in range(len(time_step_list)):
            sd_runtime = profiler.get(f"stable_diffusion_run_for_inference_{i}")
            tt_guide_runtime = profiler.get(f"tt_guide_runtime_{i}")
            ttnn_scheduler_runtime = profiler.get(f"ttnn_scheduler_run_{i}")
            logger.info(f"stable_diffusion {experiment_name}-{i} unet inference time: {sd_runtime}")
            logger.info(f"stable_diffusion {experiment_name}-{i} tt-guide runtime: {tt_guide_runtime}")
            logger.info(f"stable_diffusion {experiment_name}-{i} ttnn-scheduler runtime: {ttnn_scheduler_runtime}")
        profiler.clear()


@skip_for_grayskull()
@pytest.mark.parametrize(
    "num_prompts, num_inference_steps, image_size",
    (
        (
            1,
            2,
...
def test_perf_bare_metal(device, input_path, num_prompts, num_inference_steps, image_size):
    if is_grasykull(device):
        pytest.skip("not tested on GS")

    return run_demo_inference(device, input_path, num_prompts, num_inference_steps, image_size)
...
def test_demo_diffusiondb(device, image_size):
    return run_demo_inference_diffusiondb(device, image_size)
