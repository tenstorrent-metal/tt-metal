...
def run_test_FalconModel_inference(
    ...
    configuration = transformers.FalconConfig.from_pretrained(model_version, low_cpu_mem_usage=True)
    # model = transformers.models.falcon.modeling_falcon.FalconForCausalLM(configuration).eval()
    model = transformers.models.falcon.modeling_falcon.FalconForCausalLM(configuration).eval()
    ...
    pytorch_out, pytorch_layer_present = model(input_ids=model_input, past_key_values=past_key_values, use_cache=True)
    ...
    for i in range(num_layers):
        ...
        assert_with_pcc(pytorch_layer_pres[0].squeeze(1), tt_layer_pres[0].to(pytorch_layer_present[0].dtype), pcc)
        assert_with_pcc(pytorch_layer_pres[1].squeeze(1), tt_layer_pres[1].to(pytorch_layer_present[1].dtype), pcc)
