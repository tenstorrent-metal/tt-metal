...
DEVICE_SIDE_EXCEPTION_PROGRAMMING_EXAMPLE_ENTRIES = (
    TestEntry("programming_examples/device_side_exceptions", "programming_examples/device_side_exceptions", "dassert_example device_side_exceptions.log"),
    TestEntry("programming_examples/device_side_exceptions", "programming_examples/device_side_exceptions", "dthrow_example device_side_exceptions.log"),
    TestEntry("programming_examples/device_side_exceptions", "programming_examples/device_side_exceptions", "out_of_bounds_read_from_dram device_side_exceptions.log"),
    TestEntry("programming_examples/device_side_exceptions", "programming_examples/device_side_exceptions", "out_of_bounds_read_from_l1 device_side_exceptions.log"),
    TestEntry("programming_examples/device_side_exceptions", "programming_examples/device_side_exceptions", "read_from_invalid_noc_coord device_side_exceptions.log"),
    TestEntry("programming_examples/device_side_exceptions", "programming_examples/device_side_exceptions", "read_from_reserved_base_in_l1 device_side_exceptions.log"),
)
...
def get_device_side_exception_programming_example_entries():
    return list(
        DEVICE_SIDE_EXCEPTION_PROGRAMMING_EXAMPLE_ENTRIES
    )
...
def run_device_side_exceptions_programming_example(device_side_exceptions_programming_example_entries, timeout):
    seed = time.time()

    random.seed(seed)
    random.shuffle(programming_example_entries)
    print(f"SHUFFLED DEVICE SIDE EXCEPTION PROGRAMMING EXAMPLES - Using order generated by seed {seed}")

    test_and_status_entries = dict()
    for test_entry in device_side_exceptions_programming_example_entries:
        test_type, log_file = test_entry.extra_params.split(' ')
        if os.path.isfile(log_file):
            os.remove(log_file)

        result = run_single_programming_example(test_entry, timeout)
        if filecmp.cmp(log_file, f"{os.environ['TT_METAL_HOME']}/tt_metal/programming_examples/device_side_exceptions/debug_logs/{test_type}.log"):
            result.returncode = 0

        if os.path.isfile(log_file):
            os.remove(log_file)

        test_and_status_entries[test_entry] = result

    return test_and_status_entries
...
if __name__ == "__main__":
    ...
    device_side_exceptions_programming_example_entries = get_device_side_exception_programming_example_entries()

    pe_test_report = run_programming_examples(programming_example_entries, timeout)
    dsa_pe_test_report = run_device_side_exceptions_programming_example(device_side_exceptions_programming_example_entries, timeout)

    tt_metal_test_entries = get_tt_metal_test_entries()

    llb_test_report = run_tt_metal_tests(tt_metal_test_entries, timeout)

    test_report = {**llb_test_report, **pe_test_report, **dsa_pe_test_report}
