...
@pytest.mark.parametrize("functional_vit", [ttnn_optimized_vit])
...
def test_accuracy(
    ...
    for iter in range(iterations):
        ...
        if functional_vit == ttnn_functional_vit:
            tt_inputs = ttnn.from_torch(inputs, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
        elif functional_vit == ttnn_optimized_vit:
            tt_inputs = ttnn.from_torch(inputs, dtype=ttnn.bfloat8_b, layout=ttnn.TILE_LAYOUT, device=device)
