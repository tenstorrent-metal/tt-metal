...
from models.experimental.functional_vit.tt import ttnn_optimized_vit_highres_mixed
...
def test_vit_patch_embeddings(device, model_name, batch_size, image_size, image_channels):
    ...
    parameters = preprocess_model_parameters(
        ...
        device=device,
        custom_preprocessor=ttnn_optimized_vit_highres_mixed.custom_preprocessor,
    )

    pixel_values = ttnn.from_torch(torch_pixel_values, dtype=ttnn.bfloat8_b, layout=ttnn.TILE_LAYOUT, device=device)

    output = ttnn_optimized_vit_highres_mixed.vit_patch_embeddings(
        config,
        pixel_values,
...
def test_vit_embeddings(device, model_name, batch_size, image_size, image_channels):
    ...
    output = ttnn_optimized_vit_highres_mixed.vit_embeddings(
        config,
        pixel_values,
        position_embeddings,
        cls_token,
...
def test_vit_attention(device, model_name, batch_size, sequence_size):
    ...
    output = ttnn_optimized_vit_highres_mixed.vit_attention(
        config,
        hidden_states,
...
def test_vit_intermediate(device, model_name, batch_size, sequence_size):
    ...
    output = ttnn_optimized_vit_highres_mixed.vit_intermediate(
        hidden_states,
        parameters=parameters,
    )
    output = ttnn.to_torch(output)
...
def test_vit_output(device, model_name, batch_size, sequence_size):
    ...
    output = ttnn_optimized_vit_highres_mixed.vit_output(
        config,
        intermediate,
        residual,
        parameters=parameters,
...
def test_vit_layer(device, model_name, batch_size, sequence_size):
    ...
    output = ttnn_optimized_vit_highres_mixed.vit_layer(
        config,
        hidden_states,
        attention_mask,
        parameters=parameters,
...
@pytest.mark.parametrize("model_name", ["google/vit-base-patch16-224"])
@pytest.mark.parametrize("batch_size", [8])
@pytest.mark.parametrize("sequence_size", [600])  # 448
def test_vit_encoder(device, model_name, batch_size, sequence_size):
    ...
    output = ttnn_optimized_vit_highres_mixed.vit_encoder(
        config,
        hidden_states,
        head_masks,
        parameters=parameters,
...
def test_vit(device, model_name, batch_size, image_size, image_channels, sequence_size):
    ...
    output = ttnn_optimized_vit_highres_mixed.vit(
        config,
        pixel_values,
        head_masks,
        position_embeddings,
