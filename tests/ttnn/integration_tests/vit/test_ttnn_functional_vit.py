...
def test_vit_embeddings(device, model_name, batch_size, image_size, image_channels):
    ...
    torch_cls_token = torch.nn.Parameter(model_state_dict["cls_token"]).to(torch.bfloat16)
    torch_position_embeddings = torch.nn.Parameter(model_state_dict["position_embeddings"]).to(torch.bfloat16)
...
def test_vit_encoder(device, model_name, batch_size, sequence_size):
    ...
    assert_with_pcc(torch_output, output, 0.994)
...
def test_vit(device, model_name, batch_size, image_size, image_channels):
    ...
    torch_output, *_ = model(torch_pixel_values).pooler_output
    ...
    assert_with_pcc(torch_output, output[0][0], 0.96)
