...
#     "fidelity", [ttl.tensor.MathFidelity.LoFi, ], ids=["LoFi", ]
# )
...
def test_bert_linear(
    ...
    shard_halo = False
    shard_spec = ttl.tensor.ShardSpec(shard_grid, shard_shape, ttl.tensor.ShardOrientation.COL_MAJOR, shard_halo)

    in0_t = ttl.tensor.Tensor(
        in0, ttl.tensor.DataType.BFLOAT8_B, device, ttl.tensor.Layout.TILE, sharded_mem_config, shard_spec
    )
    in0_t = in0_t.to(device, sharded_mem_config, shard_spec)
