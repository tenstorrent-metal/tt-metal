...
void kernel_main() {
    ...
    // in0 block args
    uint32_t in0_block_h_rows                   = get_arg_val<uint32_t>(3);
    uint32_t in0_block_num_tiles                = get_arg_val<uint32_t>(4);
    uint32_t in0_bank_unit_size                 = get_arg_val<uint32_t>(5);

    // in0 row size info
    uint32_t in0_row_size                      = get_arg_val<uint32_t>(6); // num bytes in a row
    uint32_t in0_partial_row_size              = get_arg_val<uint32_t>(7); // num bytes in a row that fit within one in0_block_w
    ...
    for(uint32_t b = 0; b < num_blocks; b++) {
        ...
        for (uint32_t h = 0; h < in0_block_h_rows / 32; h++) {
            for (uint32_t i = 0; i < 32; i++) {
                ...
                noc_async_read(in0_row_noc_addr, l1_write_addr, partial_row_size);
                row_bank_id++;
            }
            noc_async_read_barrier();
        }
        row_offset = (row_offset + partial_row_size) % row_size;

        // If we have iterated through an entire row of blocks, only then do we increment our
        // row bank id
        row_bank_id = in0_tensor_start_row_id + mulsi3(((b + 1) % in0_num_blocks_w) == 0, in0_block_h_rows);
