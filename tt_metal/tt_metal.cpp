...
const Device &CreateDevice(chip_id_t device_id, const std::vector<uint32_t>& l1_bank_remap) {
    ...
    static std::vector<std::unique_ptr<Device>> device_vec ( Device::detect_num_available_devices() );
    std::once_flag f;
    std::call_once(f, [&](){
        for ( unsigned i = 0; i < device_vec.size(); i++){
            device_vec[i] = std::make_unique<Device> ( i, l1_bank_remap);
        } });

    const char *TT_METAL_SLOW_DISPATCH_MODE = std::getenv("TT_METAL_SLOW_DISPATCH_MODE");
    if (TT_METAL_SLOW_DISPATCH_MODE == nullptr) {
        detail::GLOBAL_CQ = std::make_unique<CommandQueue>( *device_vec[0]);

    }
    return *device_vec[device_id];
}
