...
class EnqueueReadBufferCommand : public Command {
   ...
   public:
    ...
    EnqueueReadBufferCommand(const Device &device, Buffer& buffer, vector<uint32_t>& dst, SystemMemoryWriter& writer);
...
class EnqueueWriteBufferCommand : public Command {
   ...
   public:
    EnqueueWriteBufferCommand(const Device &device, Buffer& buffer, vector<uint32_t>& src, SystemMemoryWriter& writer);
