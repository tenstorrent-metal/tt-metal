...
ProgramMap ConstructProgramMap(const Device* device, Program& program, uint8_t command_queue_id) {
    /*
        TODO(agrebenisan): Move this logic to compile program
    */
    uint8_t num_hw_cqs = device->num_hw_cqs();
    tt_cxy_pair dispatch_logical_location = dispatch_core_manager::get(num_hw_cqs).command_dispatcher_core(device->id(), tt::Cluster::instance().get_assigned_channel_for_device(device->id()), command_queue_id);
    CoreCoord dispatch_logical_core(dispatch_logical_location.x, dispatch_logical_location.y);
...
CommandQueue::CommandQueue(Device* device, uint32_t id): manager(*device->sysmem_manager) {
    this->device = device;
    this->id = id;

    chip_id_t mmio_device_id = tt::Cluster::instance().get_associated_mmio_device(device->id());
    this->size_B = tt::Cluster::instance().get_host_channel_size(mmio_device_id, tt::Cluster::instance().get_assigned_channel_for_device(device->id())) / device->num_hw_cqs();
}
...
void CommandQueue::enqueue_program(Program& program, bool blocking) {
    ...
    if (not program_to_buffer.count(program_id)) {
        stall = true;
        ProgramMap program_to_device_map = ConstructProgramMap(this->device, program, this->id);
...
void CommandQueue::finish() {
    ...
    tt::log_debug(tt::LogDispatch, "Finish for channel {}", this->id);
