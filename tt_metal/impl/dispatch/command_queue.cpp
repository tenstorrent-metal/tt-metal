...
EnqueueReadBufferCommand::EnqueueReadBufferCommand(
    const Device &device, Buffer& buffer, vector<uint32_t>& dst, SystemMemoryWriter& writer) :
...
EnqueueWriteBufferCommand::EnqueueWriteBufferCommand(
    const Device &device, Buffer& buffer, vector<uint32_t>& src, SystemMemoryWriter& writer) :
...
FinishCommand::FinishCommand(const Device &device, SystemMemoryWriter& writer) : writer(writer) { this->device = device; }
...
EnqueueWrapCommand::EnqueueWrapCommand(const Device &device, SystemMemoryWriter& writer) : writer(writer) {
    this->device = device;
}
...
CommandQueue::CommandQueue(const Device &device): sysmem_writer(device) {
