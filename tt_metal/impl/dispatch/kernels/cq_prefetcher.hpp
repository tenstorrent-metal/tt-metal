        ...
        while (this->num_events) {
            ...
            if constexpr (write_to_completion_queue) {
                ...
                *reinterpret_cast<volatile tt_l1_ptr uint32_t*>(event_addr) = event;
                ...
                volatile tt_l1_ptr uint32_t* push_semaphore_addr = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(get_semaphore(0));

                volatile tt_l1_ptr uint32_t* command_ptr = reinterpret_cast<volatile tt_l1_ptr uint32_t*>(L1_UNRESERVED_BASE);
                ...
                relay_command<L1_UNRESERVED_BASE, eth_l1_mem::address_map::ERISC_APP_RESERVED_BASE, 0>(false, this->push_noc_encoding); // SRC router has one cmd slot
