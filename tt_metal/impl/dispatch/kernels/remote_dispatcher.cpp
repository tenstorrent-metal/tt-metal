...
void kernel_main() {
    ...
    while (true) {
        ...
        const db_cb_config_t *remote_producer_db_cb_config = (db_cb_config_t *)(CQ_CONSUMER_CB_BASE + (db_rx_buf_switch * l1_db_cb_addr_offset));
        ...
        if (is_program) {
            ...
            // TODO: Uplift this!
            // if the dst buffer type is system memory then we need to produce data for the remote signaller to consume
            write_buffers<0>(
                db_cb_config,
                remote_producer_db_cb_config,
                command_ptr,
