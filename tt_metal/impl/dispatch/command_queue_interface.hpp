...
class SystemMemoryManager {
   ...
   public:
    ...
    void completion_queue_wait_front(const uint8_t cq_id, std::atomic<bool>& exit_condition) const {
        uint32_t write_ptr_and_toggle;
        uint32_t write_ptr;
    ...
    void completion_queue_reserve_back(uint32_t cmd_size_B, const uint8_t cq_id) {
        uint32_t cmd_size_16B = align(cmd_size_B, 32) >> 4;
        SystemMemoryCQInterface& cq_interface = this->cq_interfaces[cq_id];
        // if (cq_interface.next_completion_fifo_wr_ptr + cmd_size_16B > cq_interface.completion_fifo_limit) {
    ...
    void completion_queue_push_back(uint32_t push_size_B, const uint8_t cq_id) {
        uint32_t push_size_16B = align(push_size_B, 32) >> 4;
        SystemMemoryCQInterface& cq_interface = this->cq_interfaces[cq_id];
