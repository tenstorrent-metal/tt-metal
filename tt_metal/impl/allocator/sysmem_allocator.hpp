...
class SysmemAllocator : public Allocator {
   public:
    SysmemAllocator(const tt_SocDescriptor &soc_desc);

    ~SysmemAllocator() {}

    // TODO: Update copy/move semantics
    SysmemAllocator(const SysmemAllocator &other) { }
    SysmemAllocator& operator=(const SysmemAllocator &other) { return *this; }

    SysmemAllocator(SysmemAllocator &&other) { }
    SysmemAllocator& operator=(SysmemAllocator &&other) { return *this; }
   ...
   private:
    ...
    BankIdToRelativeAddress allocate_sysmem_buffer(uint32_t size_bytes);
    ...
    std::unique_ptr<allocator::Algorithm> sysmem_manager;
