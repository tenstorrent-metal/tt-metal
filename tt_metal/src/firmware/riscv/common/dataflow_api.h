...
void setup_cb_read_write_interfaces() {
    ...
    for (uint32_t cb_id = 0; cb_id < NUM_CIRCULAR_BUFFERS; cb_id++) {
        ...
        // Read interface
        cb_read_interface[cb_id].fifo_limit = fifo_addr + fifo_size - 1;
 ...
 */
FORCE_INLINE
void cb_push_back(const uint32_t operand, const std::int32_t num_pages) {
 ...
 */
FORCE_INLINE
void cb_pop_front(uint32_t operand, std::int32_t num_pages) {
    volatile uint32_t* pages_acked_ptr = get_cb_tiles_acked_ptr(operand);
    pages_acked_ptr[0] += num_pages;

    std::uint32_t num_words = num_pages * cb_read_interface[operand].fifo_page_size;
...
inline __attribute__((always_inline)) uint32_t get_write_ptr(std::int32_t operand) {
    ...
    std::uint32_t wr_ptr_bytes = cb_write_interface[operand].fifo_wr_ptr << 4;
    return wr_ptr_bytes;
}
 ...
 * | Argument  | Description                          | Type     | Valid Range | Required |
 * |-----------|--------------------------------------|----------|---------------------------------------------------------------------------------------------------|----------|
 * | operna    | The index of the cirular buffer (CB) | uint32_t | 0 to 31     | True     |
 */
inline __attribute__((always_inline)) uint32_t get_read_ptr(std::int32_t operand) {
    ...
    std::uint32_t rd_ptr_bytes = cb_read_interface[operand].fifo_rd_ptr << 4;
    return rd_ptr_bytes;
}
