...
void kernel_main() {
    ...
    uint32_t input_l1_addr = get_write_ptr(input_cb_id_index);
    ...
    uint32_t output_l1_addr = get_read_ptr(num_non_zeros_cb_index);
    volatile tt_l1_ptr int* output_addr_ptr = reinterpret_cast<volatile tt_l1_ptr int*>(output_l1_addr);
    output_addr_ptr[0] = num_non_zero_indices;
    uint32_t dst_noc_addr = get_noc_addr(0, out0);
    noc_async_write(l1_addr, dst_noc_addr, 32);

    dst_noc_addr = get_noc_addr(0, out1);
    noc_async_write(indices_cb_id_index_addr, dst_noc_addr, aligned_elements*element_size);
