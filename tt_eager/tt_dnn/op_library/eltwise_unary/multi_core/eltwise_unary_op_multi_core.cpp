...
operation::ProgramWithCallbacks eltwise_unary_sharded_multi_core(const Tensor &input, Tensor &output, const std::vector<UnaryWithParam> op_chain){
    ...
    if (input.memory_config().memory_layout == TensorMemoryLayout::HEIGHT_SHARDED) {
        ...
        ncores_nhw = ncores;
    }else if (input.memory_config().memory_layout == TensorMemoryLayout::WIDTH_SHARDED) {
        ncores_x = device->compute_with_storage_grid_size().x;
        ncores_nhw = ncores;
    ...
    tt_metal::KernelHandle unary_reader_kernel_id = tt_metal::CreateKernel(
        program,
        "tt_eager/tt_dnn/op_library/eltwise_unary/kernels/dataflow/reader_unary_op.cpp",
        all_cores,
    ...
    if(input.memory_config().memory_layout == TensorMemoryLayout::HEIGHT_SHARDED){
        ...
        TT_FATAL(false, "Only width, height and block memory is supported by this fuction");
    }
    ...
    (
        ...
        for (uint32_t i = 0, num_tiles_written = 0; i < ncores_nhw; i++){
            CoreCoord core = {i / ncores_x, i % ncores_nhw};
...
operation::ProgramWithCallbacks eltwise_unary_multi_core(const Tensor &a, Tensor &output, const std::vector<UnaryWithParam> op_chain) {
    ...
    if(a.memory_config().memory_layout == TensorMemoryLayout::HEIGHT_SHARDED || a.memory_config().memory_layout == TensorMemoryLayout::BLOCK_SHARDED || a.memory_config().memory_layout == TensorMemoryLayout::WIDTH_SHARDED){
        return eltwise_unary_sharded_multi_core(a, output, op_chain);
    }
