...
std::vector<Tensor> _rdiv_bw(const Tensor& grad, const Tensor& input, float scalar, string round_mode, const MemoryConfig& output_mem_config) {
    ...
    if (round_mode=="None"){
        Tensor result = where(nez(input), mul(neg(grad, output_mem_config) , (mul_unary(recip(square(input, output_mem_config)), scalar, output_mem_config)), std::nullopt, output_mem_config), t_nan);
        if (scalar>0){
            result = where(logical_and(eqz(input), ltz(grad)), t_inf, result);
            result = where(logical_and(eqz(input), gtz(grad)), -t_inf, result);
        }
        else if (scalar<0){
            result = where(logical_and(eqz(input), ltz(grad)), -t_inf, result);
            result = where(logical_and(eqz(input), gtz(grad)), t_inf, result);
...
std::vector<Tensor> _rpow_bw(const Tensor& grad, const Tensor& input, float exponent, const MemoryConfig& output_mem_config) {
    ...
    if (exponent != 0.0){
        ...
        grad_result = where(ltz(input), t_nan, grad_result);
    }
    grad_tensor.emplace_back(grad_result);
    return grad_tensor;
}
