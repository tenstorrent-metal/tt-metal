...
inline Tensor interleaved_to_sharded(const Tensor &input_tensor, CoreCoord grid_size, std::array<uint32_t, 2> shard_shape, TensorMemoryLayout shard_scheme, ShardOrientation shard_orientation, std::optional<const DataType> output_dtype=std::nullopt) {
    ...
    return operation::run(Sharded{.grid_size=grid_size, .shard_spec=shard_spec, .sharded_op_type=ShardedOpType::InterleavedToSharded, .output_mem_config=sharded_mem_config, .output_dtype=output_dtype.value_or(input_tensor.dtype())}, {input_tensor}).at(0);
}

inline Tensor sharded_to_interleaved(const Tensor &input_tensor, const MemoryConfig& output_mem_config = operation::DEFAULT_OUTPUT_MEMORY_CONFIG, std::optional<const DataType> output_dtype=std::nullopt) {
    TT_FATAL(input_tensor.shard_spec().has_value());
    auto shard_spec = input_tensor.shard_spec().value();
    return operation::run(Sharded{.grid_size=input_tensor.device()->compute_with_storage_grid_size(), .shard_spec=shard_spec, .sharded_op_type=ShardedOpType::ShardedToInterleaved, .output_mem_config=output_mem_config, .output_dtype=output_dtype.value_or(input_tensor.dtype())}, {input_tensor}).at(0);
}
