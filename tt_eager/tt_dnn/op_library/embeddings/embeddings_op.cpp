...
operation::ProgramWithCallbacks embeddings_tilized(
    ...
    tt_metal::CircularBufferConfig cb_src0_config =
        tt_metal::CircularBufferConfig(
            num_tiles_per_block * input_single_tile_size, {{src0_cb_index, input_cb_data_format}})
    ...
    tt_metal::CircularBufferConfig cb_src1_config =
        tt_metal::CircularBufferConfig(round_up_to_mul32(index_size), {{src1_cb_index, index_cb_data_format}})
            .set_page_size(src1_cb_index, index_size);
    auto cb_src1 = tt_metal::CreateCircularBuffer(program, all_cores, cb_src1_config);

    uint32_t output_cb_index = 16;  // output operands start at index 16
    tt_metal::CircularBufferConfig cb_output_config =
        tt_metal::CircularBufferConfig(
            num_tiles_per_block * output_single_tile_size, {{output_cb_index, output_cb_data_format}})
    ...
    auto reader_kernel_id = tt_metal::CreateKernel(
        ...
        tt_metal::DataMovementConfig{
            .processor = risc_procs[0], .noc = noc_ports[0], .compile_args = embedding_compile_time_args});
    ...
    auto writer_kernel_id = tt_metal::CreateKernel(
        ...
        all_cores,
        tt_metal::DataMovementConfig{
            .processor = risc_procs[1], .noc = noc_ports[1], .compile_args = writer_compile_time_args});
