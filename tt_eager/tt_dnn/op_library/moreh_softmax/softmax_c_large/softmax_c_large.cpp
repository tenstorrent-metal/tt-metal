...
operation::ProgramWithCallbacks moreh_softmax_c_large(const Tensor &input, const Tensor &output, uint32_t dim, const CoreRange core_range, const MorehSoftmaxOp op, const DeviceComputeKernelConfig compute_kernel_config) {
    ...
    auto arc = input.device()->arch();
    auto [math_fidelity, math_approx_mode, fp32_dest_acc_en, packer_l1_acc] = get_compute_kernel_config_args(arc, compute_kernel_config);
    ...
    CreateCircularBuffer(
        ...
        data_format,
        {
            ...
            {CB::c_intermed2, 1, fp32_dest_acc_en ? tt::DataFormat::Float32 : data_format},   // add
            // {CB::c_in0, 2},         // input
            // {CB::c_out0, 2},        // output
