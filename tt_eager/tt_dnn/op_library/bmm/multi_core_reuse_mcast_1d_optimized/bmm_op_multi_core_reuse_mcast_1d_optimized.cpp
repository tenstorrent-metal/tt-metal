...
operation::ProgramWithCallbacks create_program_mcast_in0(
    ...
    if (in0_is_sharded) {
        if (in2_CB_size < in0_buffer->size()) {
            std::cout << "BMM multi core reuse mcast 1d optimized cb_src2 cb smaller than buffer" << std::endl;
        }
    ...
    if (output_is_sharded) {
        if (out_CB_size < out_buffer->size()) {
            std::cout << "BMM multi core reuse mcast 1d optimized cb_output cb smaller than buffer" << std::endl;
        }
...
operation::ProgramWithCallbacks create_program_mcast_in1(
    ...
    tt_metal::CircularBufferConfig src0_cb_config = tt_metal::CircularBufferConfig(in0_CB_size, {{src0_cb_index, in0_data_format}})
		.set_page_size(src0_cb_index, in0_single_tile_size);
    if (in0_is_sharded) {
        if (in0_CB_size < in0_buffer->size()) {
            std::cout << "BMM multi core reuse mcast 1d optimized cb_src0 cb smaller than buffer" << std::endl;
        }
