...
void MAIN {
    ...
    mm_block_init_sync_full(in0_cb_id, in1_cb_id, out_cb_id);
    ...
    for (uint32_t b = 0; b < batch; b++){
        ...
        for(uint32_t block = 0; block < num_blocks; block++)
        {
            ...
            int in0_index = 0; // offset into in0 block
            int in1_index = 0; // offset into in1 block
        ...
        #endif
        tile_regs_commit_sync_full();
        cb_reserve_back(mm_out_cb_id, out_subblock_num_tiles);
        tile_regs_wait();

        int start_dst_index = 0;
        matmul_pack_tile_sync_full(start_dst_index, mm_out_cb_id, out_subblock_num_tiles);

        tile_regs_release_sync_full();
