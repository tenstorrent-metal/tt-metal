...
void kernel_main() {
    ...
    *(in0_mcast_receiver_semaphore_addr_ptr) = VALID;
    ...
    #ifdef IN0_SHARDED
    uint64_t in0_start_address = get_write_ptr(cb_id_in0);  // copy start address of block, to be used for mcasting
    #endif
    #endif

    //constexpr uint32_t in0_block_size_bytes = in0_block_h * in0_block_w * in0_single_tile_size_bytes;
