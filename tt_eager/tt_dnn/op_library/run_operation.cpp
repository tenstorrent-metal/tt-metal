...
static std::optional<std::reference_wrapper<const Device>> get_device(const std::vector<Tensor>& input_tensors, const std::vector<std::optional<const Tensor>>& optional_input_tensors = {}) {
