...
std::vector<Tensor> SplitFusedQKVAndSplitHeads::create_output_tensors(const std::vector<Tensor>& input_tensors) const {
    ...
    if (input_tensor.is_sharded()) {
        ...
        ShardSpec shard_spec_qv = ShardSpec{.grid=all_cores, .shape={per_core_M_qv, per_core_N_qv}, .orientation=shard_orientation};
        uint32_t per_core_M_k = (num_heads / num_M_cores) * K; // 128
        uint32_t per_core_N_k = M; // 384
        ShardSpec shard_spec_k = ShardSpec{.grid=all_cores, .shape={per_core_M_k, per_core_N_k}, .orientation=shard_orientation};
