...
void kernel_main() {
    ...
    uint32_t local_eth_l1_curr_src_addr = buffer0_addr + 32;
    const uint32_t end_read_idx = input_start_idx + rem_num_pages;

    for (uint32_t input_page_idx = input_start_idx; input_page_idx < end_read_idx; ++input_page_idx) {
    ...
    // uint32_t a = reinterpret_cast<volatile tt_l1_ptr uint32_t *>(local_eth_l1_curr_src_addr - page_size)[0];

    DPRINT<<BF16(reinterpret_cast<volatile tt_l1_ptr uint16_t *>(buffer0_addr + 32)[0])<<ENDL();
    DPRINT<<BF16(reinterpret_cast<volatile tt_l1_ptr uint32_t *>(local_eth_l1_curr_src_addr - page_size)[0])<<ENDL();
    // erisc_info->unused_arg1 = a;
