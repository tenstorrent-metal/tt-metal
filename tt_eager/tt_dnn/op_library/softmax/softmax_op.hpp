...
struct SoftmaxShardedDefaultProgramConfig{
...
using SoftmaxShardedProgramConfig = std::variant<
    SoftmaxShardedDefaultProgramConfig,
...
struct Softmax {
    ...
    const tt::operations::primary::transformers::SoftmaxShardedProgramConfig program_config;
...
Tensor scale_mask_softmax_in_place(Tensor& input_tensor, std::optional<float> scale = std::nullopt, std::optional<const Tensor> mask = std::nullopt, const SoftmaxShardedProgramConfig& program_config = SoftmaxShardedDefaultProgramConfig{});
}  // namespace transformers
