...
std::vector<T> read_data_from_device(const Tensor &tensor, uint32_t size_in_bytes) {
    ...
    if (TT_METAL_DEVICE_DISPATCH_MODE != nullptr) {
        EnqueueReadBuffer(tt::tt_metal::detail::get_command_queue(tensor.device()), *device_buffer, device_data, true);
    } else {
...
inline void write_data_to_device_buffer(const BufferType<T>& data_to_write, DeviceBuffer buffer, const Shape& shape, DataType data_type, Layout layout, const MemoryConfig& memory_config) {
    ...
    const char *TT_METAL_DEVICE_DISPATCH_MODE = std::getenv("TT_METAL_DEVICE_DISPATCH_MODE");
    if (TT_METAL_DEVICE_DISPATCH_MODE != nullptr) {
        EnqueueWriteBuffer(tt::tt_metal::detail::get_command_queue(tensor.device()), *buffer, uint32_data, false);
    } else {
